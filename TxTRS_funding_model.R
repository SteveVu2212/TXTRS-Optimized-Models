...
get_funding_data <- function(
  ...
  liability_df <- get_liability_data(ee_cont_cb = ee_cont_cb, 
  ...
  for (i in 2:length(year)) {
    ...
    ben_payment_new[i] <- retire_ben_DB_new_est[i] + retire_ben_CB_new_est[i]
    refund_new[i] <- refund_DB_new_est[i] + refund_CB_new_est[i]
    
    #Normal cost projection
    nc_legacy[i] <- nc_rate_DB_legacy[i] * payroll_DB_legacy[i]
    nc_new[i] <- nc_rate_DB_new[i] * payroll_DB_new[i] + nc_rate_CB_new[i] * payroll_CB_new[i]
    ...
    if(payroll_DB_new[i] + payroll_CB_new[i] == 0) {
      nc_rate_new[i] <- 0
    } else {
      nc_rate_new[i] <- nc_new[i] / (payroll_DB_new[i] + payroll_CB_new[i])  
    }
    
    if(year[i] < 2024) {
      ee_nc_rate_legacy[i] <- 0.08
      ee_nc_rate_new[i] <- 0.08
    } else {
      ee_nc_rate_legacy[i] <- 0.0825
      ee_nc_rate_new[i] <- 0.0825
    }
    ...
    #Amortization contribution rates
    if (funding_policy == "statutory") {
      amo_rate_legacy[i] <- er_stat_eff_rate[i] - er_nc_rate_legacy[i]
      amo_rate_new[i] <- er_stat_eff_rate[i] - er_nc_rate_new[i]  
    } else {
      ...
      if (payroll_DB_new[i] + payroll_CB_new[i] == 0) {
        ...
        amo_rate_new[i] <- sum(amo_payment_table_new[i-1,]) / (payroll_DB_new[i] + payroll_CB_new[i])
      }
    }
    ...
    ee_nc_cont_new[i] <- ee_nc_rate_new[i] * (payroll_DB_new[i] + payroll_CB_new[i])
    
    #Admin expense amounts
    admin_exp_legacy[i] <- admin_exp_rate[i] * payroll_DB_legacy[i]
    admin_exp_new[i] <- admin_exp_rate[i] * (payroll_DB_new[i] + payroll_CB_new[i])
    
    #Employer contribution amounts
    er_nc_cont_legacy[i] <- er_nc_rate_legacy[i] * payroll_DB_legacy[i] + admin_exp_legacy[i]
    er_nc_cont_new[i] <- er_nc_rate_new[i] * (payroll_DB_new[i] + payroll_CB_new[i]) + admin_exp_new[i]
    ...
    er_amo_cont_new[i] <- amo_rate_new[i] * (payroll_DB_new[i] + payroll_CB_new[i])
    ...
    MVA[i] <- MVA_legacy[i] + MVA_new[i]
    
    #AVA legacy development
    exp_inv_income_legacy[i] <- AVA_legacy[i-1] * dr_current + net_cf_legacy[i] * dr_current/2
    exp_AVA_legacy[i] <- AVA_legacy[i-1] + net_cf_legacy[i] + exp_inv_income_legacy[i]
    asset_gain_loss_legacy[i] <- MVA_legacy[i] - exp_AVA_legacy[i]
    
    remain_defer_boy_legacy[i] <- asset_gain_loss_legacy[i] - (defer_y1_legacy[i-1] + defer_y2_legacy[i-1] + defer_y3_legacy[i-1] + defer_y4_legacy[i-1])
    aft_offset_defer_y4_legacy[i] <- ifelse(sign(remain_defer_boy_legacy[i]) == sign(defer_y4_legacy[i-1]), remain_defer_boy_legacy[i], max(0, abs(remain_defer_boy_legacy[i]) - abs(defer_y4_legacy[i-1])) * sign(remain_defer_boy_legacy[i]))
    defer_y4_legacy[i] <- ifelse(sign(aft_offset_defer_y4_legacy[i]) == sign(defer_y3_legacy[i-1]), defer_y3_legacy[i-1] * 1/2, max(0, abs(defer_y3_legacy[i-1]) - abs(aft_offset_defer_y4_legacy[i])) * sign(defer_y3_legacy[i-1]) * 1/2)
    
    aft_offset_defer_y3_legacy[i] <- ifelse(sign(aft_offset_defer_y4_legacy[i]) == sign(defer_y3_legacy[i-1]), aft_offset_defer_y4_legacy[i], max(0, abs(aft_offset_defer_y4_legacy[i]) - abs(defer_y3_legacy[i-1])) * sign(aft_offset_defer_y4_legacy[i]))
    defer_y3_legacy[i] <- ifelse(sign(aft_offset_defer_y3_legacy[i]) == sign(defer_y2_legacy[i-1]), defer_y2_legacy[i-1] * 2/3, max(0, abs(defer_y2_legacy[i-1]) - abs(aft_offset_defer_y3_legacy[i])) * sign(defer_y2_legacy[i-1]) * 2/3)
    
    aft_offset_defer_y2_legacy[i] <- ifelse(sign(aft_offset_defer_y3_legacy[i]) == sign(defer_y2_legacy[i-1]), aft_offset_defer_y3_legacy[i], max(0, abs(aft_offset_defer_y3_legacy[i]) - abs(defer_y2_legacy[i-1])) * sign(aft_offset_defer_y3_legacy[i]))
    defer_y2_legacy[i] <- ifelse(sign(aft_offset_defer_y2_legacy[i]) == sign(defer_y1_legacy[i-1]), defer_y1_legacy[i-1] * 3/4, max(0, abs(defer_y1_legacy[i-1]) - abs(aft_offset_defer_y2_legacy[i])) * sign(defer_y1_legacy[i-1]) * 3/4)
    
    aft_offset_defer_y1_legacy[i] <- ifelse(sign(aft_offset_defer_y2_legacy[i]) == sign(defer_y1_legacy[i-1]), aft_offset_defer_y2_legacy[i], max(0, abs(aft_offset_defer_y2_legacy[i]) - abs(defer_y1_legacy[i-1])) * sign(aft_offset_defer_y2_legacy[i]))
    defer_y1_legacy[i] <- aft_offset_defer_y1_legacy[i] * 4/5
    
    remain_defer_eoy_legacy[i] <- defer_y1_legacy[i] + defer_y2_legacy[i] + defer_y3_legacy[i] + defer_y4_legacy[i]
    
    
    #AVA new development
    exp_inv_income_new[i] <- AVA_new[i-1] * dr_new + net_cf_new[i] * dr_new/2
    exp_AVA_new[i] <- AVA_new[i-1] + net_cf_new[i] + exp_inv_income_new[i]
    asset_gain_loss_new[i] <- MVA_new[i] - exp_AVA_new[i]
    
    remain_defer_boy_new[i] <- asset_gain_loss_new[i] - (defer_y1_new[i-1] + defer_y2_new[i-1] + defer_y3_new[i-1] + defer_y4_new[i-1])
    aft_offset_defer_y4_new[i] <- ifelse(sign(remain_defer_boy_new[i]) == sign(defer_y4_new[i-1]), remain_defer_boy_new[i], max(0, abs(remain_defer_boy_new[i]) - abs(defer_y4_new[i-1])) * sign(remain_defer_boy_new[i]))
    defer_y4_new[i] <- ifelse(sign(aft_offset_defer_y4_new[i]) == sign(defer_y3_new[i-1]), defer_y3_new[i-1] * 1/2, max(0, abs(defer_y3_new[i-1]) - abs(aft_offset_defer_y4_new[i])) * sign(defer_y3_new[i-1]) * 1/2)
    
    aft_offset_defer_y3_new[i] <- ifelse(sign(aft_offset_defer_y4_new[i]) == sign(defer_y3_new[i-1]), aft_offset_defer_y4_new[i], max(0, abs(aft_offset_defer_y4_new[i]) - abs(defer_y3_new[i-1])) * sign(aft_offset_defer_y4_new[i]))
    defer_y3_new[i] <- ifelse(sign(aft_offset_defer_y3_new[i]) == sign(defer_y2_new[i-1]), defer_y2_new[i-1] * 2/3, max(0, abs(defer_y2_new[i-1]) - abs(aft_offset_defer_y3_new[i])) * sign(defer_y2_new[i-1]) * 2/3)
    
    aft_offset_defer_y2_new[i] <- ifelse(sign(aft_offset_defer_y3_new[i]) == sign(defer_y2_new[i-1]), aft_offset_defer_y3_new[i], max(0, abs(aft_offset_defer_y3_new[i]) - abs(defer_y2_new[i-1])) * sign(aft_offset_defer_y3_new[i]))
    defer_y2_new[i] <- ifelse(sign(aft_offset_defer_y2_new[i]) == sign(defer_y1_new[i-1]), defer_y1_new[i-1] * 3/4, max(0, abs(defer_y1_new[i-1]) - abs(aft_offset_defer_y2_new[i])) * sign(defer_y1_new[i-1]) * 3/4)
    
    aft_offset_defer_y1_new[i] <- ifelse(sign(aft_offset_defer_y2_new[i]) == sign(defer_y1_new[i-1]), aft_offset_defer_y2_new[i], max(0, abs(aft_offset_defer_y2_new[i]) - abs(defer_y1_new[i-1])) * sign(aft_offset_defer_y2_new[i]))
    defer_y1_new[i] <- aft_offset_defer_y1_new[i] * 4/5
    
    remain_defer_eoy_new[i] <- defer_y1_new[i] + defer_y2_new[i] + defer_y3_new[i] + defer_y4_new[i]
    
    
    #AVA, UAl, and funded ratio projections
    AVA_legacy[i] <- MVA_legacy[i] - remain_defer_eoy_legacy[i]
    AVA_new[i] <- MVA_new[i] - remain_defer_eoy_new[i]
...
# 
# test <- get_funding_data()
# test2 <- get_funding_data(return_scen = "recur_recession")
# 
# baseline_7 <- output
# baseline_recession <- output
# baseline_recur_recession <- output
# 
# cola_7 <- output
# cola_recession <- output
# cola_recur_recession <- output
# 
# output_list <- list("Baseline-7" = baseline_7,
#                     "Baseline-recession" = baseline_recession,
#                     "Baseline-recur recession" = baseline_recur_recession,
#                     "COLA-7" = cola_7,
#                     "COLA-recession" = cola_recession, 
#                     "COLA-recur recession" = cola_recur_recession)
# 
# 
# 
# write.xlsx(output_list, file = "Texas TRS analysis.xlsx", overwrite = T)
